...
export default function CommunityTips() {
  ...
  const { data: notice, isPending } = useGetCommunity("tips");
  const { mutate: deleteNotice } = useDeleteCommunity("tips");
    ...
    >
      <div className="flex items-center justify-end mb-4 w-full">
        <Button
          ...
          onClick={() => router.push("/community/create?tab=tips")}
        >
          작성하기
        </Button>
      </div>
      ...
      <Card>
        <CardContent className="p-3 sm:p-6">
          {isMounted && !isPending ? (
            <DataTable
              key={`community-tips-table-${noticeData.length}`}
