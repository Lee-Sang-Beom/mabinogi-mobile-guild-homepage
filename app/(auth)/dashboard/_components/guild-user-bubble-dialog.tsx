...
interface GuildUserBubbleDialogProps {
    ...
    <Dialog open={!!selectedUser} onOpenChange={(open) => !open && setSelectedUser(null)}>
      <DialogContent className="sm:max-w-md bg-background/80 backdrop-blur-sm ">
        <DialogHeader>
          ...
          <DialogDescription>
            <span className={"font-bold"}>{selectedUser.id}</span>님의 상세 정보입니다.</DialogDescription>
        </DialogHeader>
        {selectedUser && (
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              ...
              <div className={'flex items-center'}>
                <h3 className="font-bold text-lg flex items-center relative">
                  <span>
                    {isBadge ? <Award className={"absolute top-[-20px] left-[-15px] text-primary w-5 h-5"} /> : null}
                  </span>
                  <span>
                     {selectedUser.name}
                  </span>
                </h3>
                <p className={cn("text-muted-foreground", isBadge ? "ml-1": "")}>
            ...
            <div className="space-y-2">
              <div className="flex justify-between">
                ...
                <span className="font-medium"> {guildRoleOptions.find((role) => role.value === selectedUser.role)!.name}</span>
              </div>
            </div>
            <div className="space-y-2">
              <div className="flex justify-between">
