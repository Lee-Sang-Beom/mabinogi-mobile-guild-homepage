...
export default function ParticipateScheduleDialogProps({
  ...
  const onParticipate = (values: ParticipatePartyFormSchema) => {
    ...
    const isAlreadyParticipating = (
      selectSchedule.participateEtcUser ?? []
    ).some(
      (u) =>
        u.participateUserParentDocId === postData.participateUserParentDocId
    );

    // 파티에 가입되어 있지 않은 상태에서 onParticipate가 실행된다면, 파티신청 시 파티원 구인이 마감되었는지 확인해야 함
    if (
      !isAlreadyParticipating &&
      selectSchedule.maxParticipateCount <=
        (selectSchedule.participateEtcUser.length || 0) + 1
    ) {
  ...
  useEffect(() => {
    ...
    const alreadyIsParticipate = scheduleParticipateUserList.find(
      (participateUser) =>
        participateUser.participateUserParentDocId === user.docId
    );
  ...
  return (
    <Dialog
      open={isParticipateDialogOpen}
      onOpenChange={setIsParticipateDialogOpen}
    >
      <DialogContent className="sm:max-w-[480px] max-h-[500px] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>파티 가입 관리</DialogTitle>
          <DialogDescription>
            등록된 파티 정보와 파티 가입 및 탈퇴를 진행할 수 있습니다.{" "}
          </DialogDescription>
        </DialogHeader>

        <Form {...participateScheduleForm}>
          <form
            onSubmit={participateScheduleForm.handleSubmit(onParticipate)}
              ...
              <>
                    ...
                    >
                      ...
                      <div className="flex flex-col space-y-2 text-sm">
                        <p className="flex items-center gap-2">
                          ...
                          <strong>출발 일정:</strong> {selectSchedule.date}{" "}
                          {selectSchedule.time}
                        </p>
                        <p className="flex items-center gap-2">
                        ...
                        <p className="flex items-center gap-2">
                          ...
                          <strong>최대 파티원 수:</strong>{" "}
                          {selectSchedule.maxParticipateCount}명
                        </p>
                      </div>
                    </motion.div>
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        <motion.div whileHover={{ scale: 1.05 }}>
                          ...
                          >
                            {(() => {
                              const IconComponent =
                                JobClassIcons[
                                  selectSchedule.participateWriteUser
                                    .participateUserJob
                                ] || Sword;
                              return (
                                <IconComponent
                            ...
                            <p>
                              ...
                              <span className="ml-1 text-xs text-sky-500 font-semibold">
                                (파티장)
                              </span>
                            </p>
                          </Badge>
                        ...
                        {selectSchedule.participateEtcUser.map((member) => {
                          const isMe =
                            member.participateUserParentDocId === user.docId;
                          return (
                            <motion.div
                              key={member.participateUserDocId}
                              whileHover={{ scale: 1.05 }}
                            >
                              <Badge
                                className={cn(
                                  "flex items-center gap-1 bg-muted text-foreground border",
                                  isMe && "border-red-400"
                                )}
                                variant="outline"
                              >
                                {(() => {
                                  const IconComponent =
                                    JobClassIcons[member.participateUserJob] ||
                                    Sword;
                                  return (
                                    <IconComponent
                                      className="h-4 w-4"
                ...
                {isParticipate ? (
                  <p className={"text-sm w-full text-center"}>
                    이미 가입된 파티입니다! <br /> 파티탈퇴를 원하시면, 아래{" "}
                    <strong>파티탈퇴</strong> 버튼을 클릭해주세요.
                  </p>
                ) : (
                  <>
                    <FormField
                          ...
                          <Select
                            onValueChange={(value) => {
                              ...
                              if (selectedCharacter) {
                                ...
                                participateScheduleForm.setValue(
                                  "participateUser.participateUserJob",
                                  selectedCharacter.participateUserJob
                                );
                              }
                            }}
                            defaultValue={
                              field.value?.participateUserDocId || ""
                            }
                          >
                            <FormControl>
